...
BoxShape::BoxShape(const BoxShapeSettings& inSettings, ShapeResult& outResult)
    : ConvexShape(EShapeSubType::Box, inSettings, outResult), mHalfExtent(inSettings.mHalfExtent),
...
public:
    ...
    virtual float GetConvexRadius() const override { return mConvexRadius; }
...
void BoxShape::GetSupportingFace(const SubShapeID& inSubShapeID, Vec3Arg inDirection, Vec3Arg inScale,
                    ...
                    bool inUseMaterialColors, bool inDrawWireframe) const
{
    ...
    inRenderer->DrawBox(inCenterOfMassTransform * Mat44::sScale(inScale.Abs()), GetLocalBounds(),
                        inUseMaterialColors ? GetMaterial()->GetDebugColor() : inColor, DebugRenderer::ECastShadow::On,
                        draw_mode);
}
#endif // JPH_DEBUG_RENDERER
...
void BoxShape::CastRay(const RayCast& inRay, const RayCastSettings& inRayCastSettings,
...
void BoxShape::CollideSoftBodyVertices(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale,
        ...
        if (v.GetInvMass() > 0.0f)
        {
            ...
            if (clamped_point == local_pos)
            {
                ...
                if (v.UpdatePenetration(penetration))
                {
                    ...
                    // Store collision
                    v.SetCollision(Plane::sFromPointAndNormal(point, normal).GetTransformed(inCenterOfMassTransform),
            ...
            else
            {
                ...
                if (v.UpdatePenetration(penetration))
                {
                    ...
                    // Store collision
                    v.SetCollision(Plane::sFromPointAndNormal(clamped_point, normal).GetTransformed(inCenterOfMassTransform),
                                   inCollidingShapeIndex);
                }
            }
        }
...
void BoxShape::GetTrianglesStart(GetTrianglesContext& ioContext, const AABox& inBox, Vec3Arg inPositionCOM,
                                 QuatArg inRotation, Vec3Arg inScale) const
{
    new (&ioContext) GetTrianglesContextVertexList(inPositionCOM, inRotation, inScale, Mat44::sScale(mHalfExtent),
                                                   sUnitBoxTriangles, std::size(sUnitBoxTriangles), GetMaterial());
}

int BoxShape::GetTrianglesNext(GetTrianglesContext& ioContext, int inMaxTrianglesRequested, Float3* outTriangleVertices,
