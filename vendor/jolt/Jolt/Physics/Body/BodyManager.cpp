...
bool BodyManager::sGetOverrideAllowActivation() { return sOverrideAllowActivation; }

void BodyManager::sSetOverrideAllowActivation(bool inValue) { sOverrideAllowActivation = inValue; }

bool BodyManager::sGetOverrideAllowDeactivation() { return sOverrideAllowDeactivation; }

void BodyManager::sSetOverrideAllowDeactivation(bool inValue) { sOverrideAllowDeactivation = inValue; }
#endif
...
{
public:
    SoftBodyWithMotionPropertiesAndShape() { mShape.SetEmbedded(); }
...
inline void BodyManager::sDeleteBody(Body* inBody)
{
    if (inBody->mMotionProperties != nullptr)
    {
        ...
        {
            inBody->mShape = nullptr; // Release the shape to avoid assertion on shape destruction because of embedded object
                                      // with refcount > 0
...
void BodyManager::Init(uint inMaxBodies, uint inNumBodyMutexes, const BroadPhaseLayerInterface& inLayerInterface)
{
    ...
    uint num_body_mutexes =
        Clamp<uint>(GetNextPowerOf2(inNumBodyMutexes == 0 ? 2 * thread::hardware_concurrency() : inNumBodyMutexes), 1,
                    sizeof(MutexMask) * 8);
#ifdef JPH_TSAN_ENABLED
    num_body_mutexes = min(num_body_mutexes, 32U); // TSAN errors out when locking too many mutexes on the same thread, see:
...
Body* BodyManager::AllocateBody(const BodyCreationSettings& inBodyCreationSettings) const
{
    ...
    if (inBodyCreationSettings.HasMassProperties())
    {
        ...
        mp->SetLinearVelocity(
            inBodyCreationSettings
                .mLinearVelocity); // Needs to happen after setting the max linear/angular velocity and setting allowed DOFs
...
Body* BodyManager::AllocateSoftBody(const SoftBodyCreationSettings& inSoftBodyCreationSettings) const
{
    ...
    body->SetPositionAndRotationInternal(
        inSoftBodyCreationSettings.mPosition,
        inSoftBodyCreationSettings.mMakeRotationIdentity ? Quat::sIdentity() : inSoftBodyCreationSettings.mRotation);
...
{
    JPH_ASSERT(inBody->GetID().IsInvalid(),
               "This function should only be called on a body that doesn't have an ID yet, use DestroyBody otherwise");
...

    {
        ...
        if (idx < mBodies.size())
        {
            ...
            else
            {
                ...
                // TODO: This is O(N), see if this becomes a performance problem (don't want to put the freed bodies in a
                // double linked list)
                uintptr_t cur, next;
...
void BodyManager::ValidateFreeList() const
{
    ...
    size_t num_freed = 0;
    for (uintptr_t start = mBodyIDFreeListStart; start != cBodyIDFreeListEnd;
         start = uintptr_t(mBodies[start >> cFreedBodyIndexShift]))
    {
...
void BodyManager::AddBodyToActiveBodies(Body& ioBody)
{
    ...
    num_active_bodies.fetch_add(
        1, memory_order_release); // Increment atomic after setting the body ID so that PhysicsSystem::JobFindCollisions
                                  // (which doesn't lock the mActiveBodiesMutex) will only read valid IDs
...
#ifdef JPH_DEBUG_RENDERER
void BodyManager::Draw(const DrawSettings& inDrawSettings, const PhysicsSettings& inPhysicsSettings,
                       DebugRenderer* inRenderer, const BodyDrawFilter* inBodyFilter)
{
    ...
    for (const Body* body : mBodies)
    {
        if (sIsValidBodyPointer(body) && body->IsInBroadPhase() && (!inBodyFilter || inBodyFilter->ShouldDraw(*body)))
        {
            ...
            {
                body->mShape->DrawGetSupportFunction(inRenderer, body->GetCenterOfMassTransform(), Vec3::sOne(), color,
            ...
            if (inDrawSettings.mDrawMassAndInertia && body->IsDynamic())
            {
                    ...
                    !Vec3::sEquals(mp->GetInverseInertiaDiagonal(), Vec3::sZero()).TestAnyXYZTrue())
                {
                    ...
                    inRenderer->DrawWireBox(body->GetCenterOfMassTransform() * Mat44::sRotation(mp->GetInertiaRotation()),
                                            AABox(-0.5f * box_size, 0.5f * box_size), Color::sOrange);

                    // Draw mass
                    inRenderer->DrawText3D(body->GetCenterOfMassPosition(), StringFormat("%.2f", (double)mass),
                                           Color::sOrange, 0.2f);
                }
            }
            ...
            if (inDrawSettings.mDrawSleepStats && body->IsDynamic() && body->IsActive())
            {
                ...
                uint8 g = uint8(Clamp(255.0f * body->mMotionProperties->mSleepTestTimer / inPhysicsSettings.mTimeBeforeSleep,
                                      0.0f, 255.0f));
                Color sleep_color = Color(0, 255 - g, g);
                inRenderer->DrawText3D(body->GetCenterOfMassPosition(), text, sleep_color, 0.2f);
                for (int i = 0; i < 3; ++i)
                    inRenderer->DrawWireSphere(JPH_IF_DOUBLE_PRECISION(body->mMotionProperties->GetSleepTestOffset() +)
                                                   body->mMotionProperties->mSleepTestSpheres[i]
                                                       .GetCenter(),
                                               body->mMotionProperties->mSleepTestSpheres[i].GetRadius(), sleep_color);
            }
...
{
    // If this is the first time we flip the collision cache invalid flag, we need to add it to an internal list to ensure we
    // reset the flag at the end of the physics update
...
void BodyManager::ValidateContactCacheForAllBodies()
{
    ...
    {
        // The body may have been removed between the call to InvalidateContactCacheForBody and this call, so check if it
        // still exists
...
void BodyManager::ValidateActiveBodyBounds()
{
    ...
    for (uint type = 0; type < cBodyTypeCount; ++type)
        for (BodyID *id = mActiveBodies[type],
                    *id_end = mActiveBodies[type] + mNumActiveBodies[type].load(memory_order_relaxed);
             id < id_end; ++id)
        {
