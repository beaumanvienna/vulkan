...
// Distance for each LOD level, these are tweaked for an object of approx. size 1. Use the lod scale to scale these
// distances.
...
DebugRenderer::Triangle::Triangle(Vec3Arg inV1, Vec3Arg inV2, Vec3Arg inV3, ColorArg inColor, Vec3Arg inUVOrigin,
...
void DebugRenderer::DrawWireUnitSphereRecursive(RMat44Arg inMatrix, ColorArg inColor, Vec3Arg inDir1, Vec3Arg inDir2,
...
void DebugRenderer::Create8thSphereRecursive(Array<uint32>& ioIndices, Array<Vertex>& ioVertices, Vec3Arg inDir1,
                                             uint32& ioIdx1, Vec3Arg inDir2, uint32& ioIdx2, Vec3Arg inDir3, uint32& ioIdx3,
                                             const Float2& inUV, SupportFunction inGetSupport, int inLevel)
{
    if (inLevel == 0)
    {
    ...
    else
    {
        ...
        Create8thSphereRecursive(ioIndices, ioVertices, inDir1, ioIdx1, center1, idx1, center3, idx3, inUV, inGetSupport,
                                 inLevel - 1);
        Create8thSphereRecursive(ioIndices, ioVertices, center1, idx1, center2, idx2, center3, idx3, inUV, inGetSupport,
                                 inLevel - 1);
        Create8thSphereRecursive(ioIndices, ioVertices, center1, idx1, inDir2, ioIdx2, center2, idx2, inUV, inGetSupport,
                                 inLevel - 1);
        Create8thSphereRecursive(ioIndices, ioVertices, center3, idx3, center2, idx2, inDir3, ioIdx3, inUV, inGetSupport,
                                 inLevel - 1);
    }
}

void DebugRenderer::Create8thSphere(Array<uint32>& ioIndices, Array<Vertex>& ioVertices, Vec3Arg inDir1, Vec3Arg inDir2,
                                    Vec3Arg inDir3, const Float2& inUV, SupportFunction inGetSupport, int inLevel)
{
    uint32 idx1 = 0xffffffff;
    uint32 idx2 = 0xffffffff;
    uint32 idx3 = 0xffffffff;
...
DebugRenderer::Batch DebugRenderer::CreateCylinder(float inTop, float inBottom, float inTopRadius, float inBottomRadius,
                                                   int inLevel)
{
...
void DebugRenderer::CreateQuad(Array<uint32>& ioIndices, Array<Vertex>& ioVertices, Vec3Arg inV1, Vec3Arg inV2, Vec3Arg inV3,
                               Vec3Arg inV4)
{
    // Make room
...
void DebugRenderer::Initialize()
{
    ...
    for (int level = sMaxLevel; level >= 1; --level)
    {
        ...
        // Capsule bottom half sphere
        {
            ...
            Create8thSphere(capsule_bottom_indices, capsule_bottom_vertices, -Vec3::sAxisX(), -Vec3::sAxisY(),
                            Vec3::sAxisZ(), Float2(0.25f, 0.25f), sphere_support, level);
            Create8thSphere(capsule_bottom_indices, capsule_bottom_vertices, -Vec3::sAxisY(), Vec3::sAxisX(), Vec3::sAxisZ(),
                            Float2(0.25f, 0.75f), sphere_support, level);
            Create8thSphere(capsule_bottom_indices, capsule_bottom_vertices, Vec3::sAxisX(), -Vec3::sAxisY(),
                            -Vec3::sAxisZ(), Float2(0.25f, 0.25f), sphere_support, level);
            Create8thSphere(capsule_bottom_indices, capsule_bottom_vertices, -Vec3::sAxisY(), -Vec3::sAxisX(),
                            -Vec3::sAxisZ(), Float2(0.25f, 0.75f), sphere_support, level);
            mCapsuleBottom->mLODs.push_back(
                {CreateTriangleBatch(capsule_bottom_vertices, capsule_bottom_indices), distance});
        }
        ...
        // Capsule top half sphere
        {
            ...
            Create8thSphere(capsule_top_indices, capsule_top_vertices, Vec3::sAxisX(), Vec3::sAxisY(), Vec3::sAxisZ(),
                            Float2(0.25f, 0.75f), sphere_support, level);
            Create8thSphere(capsule_top_indices, capsule_top_vertices, Vec3::sAxisY(), -Vec3::sAxisX(), Vec3::sAxisZ(),
...
DebugRenderer::Batch DebugRenderer::CreateTriangleBatchForConvex(SupportFunction inGetSupport, int inLevel, AABox* outBounds)
{
    ...
    Create8thSphere(indices, vertices, Vec3::sAxisX(), Vec3::sAxisY(), Vec3::sAxisZ(), Float2(0.25f, 0.25f), inGetSupport,
                    inLevel);
    Create8thSphere(indices, vertices, Vec3::sAxisY(), -Vec3::sAxisX(), Vec3::sAxisZ(), Float2(0.25f, 0.75f), inGetSupport,
                    inLevel);
    Create8thSphere(indices, vertices, -Vec3::sAxisY(), Vec3::sAxisX(), Vec3::sAxisZ(), Float2(0.25f, 0.75f), inGetSupport,
                    inLevel);
    Create8thSphere(indices, vertices, -Vec3::sAxisX(), -Vec3::sAxisY(), Vec3::sAxisZ(), Float2(0.25f, 0.25f), inGetSupport,
...
void DebugRenderer::DrawBox(RMat44Arg inMatrix, const AABox& inBox, ColorArg inColor, ECastShadow inCastShadow,
...
void DebugRenderer::DrawSphere(RVec3Arg inCenter, float inRadius, ColorArg inColor, ECastShadow inCastShadow,
                               EDrawMode inDrawMode)
{
    JPH_PROFILE_FUNCTION();
...
void DebugRenderer::DrawCapsule(RMat44Arg inMatrix, float inHalfHeightOfCylinder, float inRadius, ColorArg inColor,
                                ECastShadow inCastShadow, EDrawMode inDrawMode)
{
    ...
    AABox local_bounds(Vec3(-inRadius, -inHalfHeightOfCylinder - inRadius, -inRadius),
    ...
    DrawGeometry(bottom_matrix, world_bounds, radius_sq, inColor, mCapsuleBottom, ECullMode::CullBackFace, inCastShadow,
                 inDrawMode);

    // Draw top half sphere
    RMat44 top_matrix = inMatrix * Mat44::sTranslation(Vec3(0, inHalfHeightOfCylinder, 0)) * scale_matrix;
    DrawGeometry(top_matrix, world_bounds, radius_sq, inColor, mCapsuleTop, ECullMode::CullBackFace, inCastShadow,
                 inDrawMode);

    // Draw middle part
    DrawGeometry(inMatrix * Mat44::sScale(Vec3(inRadius, inHalfHeightOfCylinder, inRadius)), world_bounds, radius_sq,
                 inColor, mCapsuleMid, ECullMode::CullBackFace, inCastShadow, inDrawMode);
}

void DebugRenderer::DrawCylinder(RMat44Arg inMatrix, float inHalfHeight, float inRadius, ColorArg inColor,
                                 ECastShadow inCastShadow, EDrawMode inDrawMode)
{
    JPH_PROFILE_FUNCTION();
    ...
    DrawGeometry(transform, mCylinder->mBounds.Transformed(transform), Square(inRadius), inColor, mCylinder,
                 ECullMode::CullBackFace, inCastShadow, inDrawMode);
}

void DebugRenderer::DrawOpenCone(RVec3Arg inTop, Vec3Arg inAxis, Vec3Arg inPerpendicular, float inHalfAngle, float inLength,
...
DebugRenderer::Geometry* DebugRenderer::CreateSwingLimitGeometry(int inNumSegments, const Vec3* inVertices)
{
    ...
    for (int i = 0; i < inNumSegments; ++i)
    {
        ...
        Vec3 normal =
            0.5f * (next_pos.Cross(pos).NormalizedOr(Vec3::sZero()) + pos.Cross(prev_pos).NormalizedOr(Vec3::sZero()));
    ...
    return new Geometry(CreateTriangleBatch(vertices_start, num_vertices, indices_start, num_indices),
                        sCalculateBounds(vertices_start, num_vertices));
}

void DebugRenderer::DrawSwingConeLimits(RMat44Arg inMatrix, float inSwingYHalfAngle, float inSwingZHalfAngle,
                                        float inEdgeLength, ColorArg inColor, ECastShadow inCastShadow, EDrawMode inDrawMode)
{
    JPH_PROFILE_FUNCTION();
...
void DebugRenderer::DrawSwingPyramidLimits(RMat44Arg inMatrix, float inMinSwingYAngle, float inMaxSwingYAngle,
                                           ...
                                           ColorArg inColor, ECastShadow inCastShadow, EDrawMode inDrawMode)
{
    ...
    if (geometry == nullptr)
    {
        ...
        // Note that this is q = Quat::sRotation(Vec3::sAxisZ(), z) * Quat::sRotation(Vec3::sAxisY(), y) with q.x set to zero
        // so we don't introduce twist This matches the calculation in SwingTwistConstraintPart::ClampSwingTwist
        ...
        for (int segment_iter = 0; segment_iter < quarter_num_segments; ++segment_iter)
            ls_vertices[tgt_vertex++] =
                get_axis(inMinSwingYAngle,
                         inMaxSwingZAngle - (inMaxSwingZAngle - inMinSwingZAngle) * segment_iter / quarter_num_segments);
        for (int segment_iter = 0; segment_iter < quarter_num_segments; ++segment_iter)
            ls_vertices[tgt_vertex++] =
                get_axis(inMinSwingYAngle + (inMaxSwingYAngle - inMinSwingYAngle) * segment_iter / quarter_num_segments,
                         inMinSwingZAngle);
        for (int segment_iter = 0; segment_iter < quarter_num_segments; ++segment_iter)
            ls_vertices[tgt_vertex++] =
                get_axis(inMaxSwingYAngle,
                         inMinSwingZAngle + (inMaxSwingZAngle - inMinSwingZAngle) * segment_iter / quarter_num_segments);
        for (int segment_iter = 0; segment_iter < quarter_num_segments; ++segment_iter)
            ls_vertices[tgt_vertex++] =
                get_axis(inMaxSwingYAngle - (inMaxSwingYAngle - inMinSwingYAngle) * segment_iter / quarter_num_segments,
                         inMaxSwingZAngle);
...
void DebugRenderer::DrawPie(RVec3Arg inCenter, float inRadius, Vec3Arg inNormal, Vec3Arg inAxis, float inMinAngle,
                            float inMaxAngle, ColorArg inColor, ECastShadow inCastShadow, EDrawMode inDrawMode)
{
    ...
    if (geometry == nullptr)
    {
        ...
        *vertices++ = {center, normal, {0, 0}, Color::sWhite};
        ...
        // Convert to triangle batch
        geometry = new Geometry(CreateTriangleBatch(vertices_start, num_vertices, indices_start, num_indices),
                                sCalculateBounds(vertices_start, num_vertices));
    }

    // Construct matrix that transforms pie into world space
    RMat44 matrix = RMat44(Vec4(inRadius * inAxis, 0), Vec4(inRadius * inNormal, 0),
...
void DebugRenderer::DrawTaperedCylinder(RMat44Arg inMatrix, float inTop, float inBottom, float inTopRadius,
                                        float inBottomRadius, ColorArg inColor, ECastShadow inCastShadow,
                                        EDrawMode inDrawMode)
{
    ...
    if (geometry == nullptr)
    {
        ...
        for (int level = sMaxLevel; level >= 1; --level)
            geometry->mLODs.push_back({CreateCylinder(inTop, inBottom, inTopRadius, inBottomRadius, level),
                                       sLODDistanceForLevel[sMaxLevel - level]});
    }
